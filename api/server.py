...
def submit_score():
    ...
    if not request.is_json:
        app.logger.warning(
            f"Non-JSON request received from {request.remote_addr}")
    ...
    app.logger.info(f"Score submission attempt for user: {name}")
    ...
    try:
        ...
        app.logger.warning(f"Invalid score/level format from {request.remote_addr} for user: {name}")
    ...
    if score < 0 or level < 0:
        app.logger.warning(f"Negative score/level from {request.remote_addr} for user: {name}")
    ...
    if not name or not secret or level is None or score is None:
        app.logger.warning(f"Missing fields in request from {request.remote_addr}")
        return jsonify({"error": {"english": "Missing fields", "czech": "Chybějící pole"}}), 400

    if not is_valid_name(name):
        app.logger.warning(f"Invalid name format from {request.remote_addr}: {name}")
        return jsonify({"error": {"english": "Invalid name format", "czech": "Neplatný formát jména"}}), 400

    for entry in data:
        if entry["name"] == name:
            if entry["secret"] != secret:
                app.logger.warning(f"Invalid secret for user {name} from {request.remote_addr}")
            ...
            if int(score) > int(entry["score"]):
                ...
                save_data(data)
                app.logger.info(f"Score updated for user {name}: {score} at level {level}")
                return jsonify({"message": {"english": "Score has been updated.", "czech": "Skóre bylo aktualizováno."}})

            app.logger.info(f"Score not beaten for user {name}: current {entry['score']}, submitted {score}")
            return jsonify({"message": {"english": "Score was not beaten, remains unchanged.", "czech": "Skóre nebylo překonáno, zůstává nezměněno."}})

    app.logger.warning(f"User not found: {name} from {request.remote_addr}")
